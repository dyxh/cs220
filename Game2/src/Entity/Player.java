...
public class Player extends MapObject {
	...
	private final int[] numFrames = { // number of frames for each animation
		2, 8, 1, 2, 4, 2, 5, 0	
	};
	...
	private static final int SHIELDING = 7;
		...
		try {
			...
			for(int i = 0; i < 7; i++) {
				...
				for(int j = 0; j < numFrames[i]; j++) {
					if(i != 6) { // if anything but scratching
	...
	private void getNextPosition() {
		...
		if(
		(currentAction == SCRATCHING || currentAction == FIREBALL || currentAction == SHIELDING) &&
		!(jumping || falling)) {
			dx = 0;
		}
